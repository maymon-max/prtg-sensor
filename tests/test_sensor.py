...
class TestSensor:
    ...
    def test_error_result(self):
        s = Sensor()
        assert s.get_error_result('Test error text') == {
            'prtg': {
                'error': 1,
    ...
    def test_result_exceptions(self):
        ...
        with pytest.raises(ValueError):
            s.add_channel('test', 0, mode='abs')
        with pytest.raises(ValueError):
            s.add_channel('test', 0, unit='Custom')
        with pytest.raises(ValueError):
            s.add_channel('test', -5.4534, show_chart=9999)
